...
Now, let us make some observations. The first thing that we notice is that the contract `Lib` declares only one state variable, but the contract `Vulnerable` declares three state variables.
